...
def search_for_tweets_with_word(twitter_df,word, display_n=10,from_column='content_min_clean', ascending=False,
                                as_md=True):
...
def df_to_md(res_df,show=False):
    from IPython.display import display, Markdown
    import pandas as pd
    df_md = res_df.copy()
    
    date_format = '%m/%d/%Y - %T'
    df_md['md_tweet'] = res_df['content'].apply(lambda x: str(x))
